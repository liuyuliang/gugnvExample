...
public class FilePathDemo {
	public static void main(String[] args) {
		try {
			// 读取文件
			List filenames = FilePathDemo.getFiles("D:/");
			if (filenames != null)
	...
	public static List getFiles(String dirname) throws Exception {
		...
		if (dir.exists()) {
			...
			// 排序
			Arrays.sort(files, new FilePathDemo.CompratorByLastModified());
			for (int i = 0; i < files.length; i++) {
				// 获取当文件最后修改时间
				String creatime = FilePathDemo.format("yyyy-MM-dd hh:mm:ss", new Date(files[i].lastModified()));
